...
test('场景：固定输出策略优先于动态目录，嵌套目录使用最具体的覆盖策略', () => {
    ...
    assert.equal(resolveAiJavaScriptOutputPolicy(exact.path, [exact], [root, nested]), exact);
    assert.equal(resolveAiJavaScriptOutputPolicy('output/extracted/a.txt', [], [root, nested]), nested);
    assert.equal(resolveAiJavaScriptOutputPolicy('output/a.txt', [], [root, nested]), root);
    assert.equal(resolveAiJavaScriptOutputPolicy('other/a.txt', [], [root, nested]), null);
});

test('场景：overwrite 为真时覆盖已保存、未保存或待删除文件，为假时统一报冲突', () => {
