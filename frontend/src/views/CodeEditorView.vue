...
function getAiAllToolDefinitions() {
  return [
    ...
    { type: "function", name: "run_javascript", description: aiToolDescription(
      ...
      "node:fs is a trimmed implementation that currently exposes only promises, readFileSync, writeFileSync, statSync, existsSync, and readdirSync. node:fs/promises is also limited. Do not call undocumented file-system methods, including mkdir, mkdirSync, rm, unlink, rename, file streams, file watchers, permission changes, or file-descriptor APIs.",
      "Do not use mkdirSync to create output directories. Declare files with known names in output_files, or declare output_directories for dynamically named child files. Only changed files within the declared output scope are extracted; all other temporary files are discarded after execution.",
