  ...
  return [
    ...
    { type: "function", name: "run_javascript", description: aiToolDescription(
      ...
      [
        ...
        "- Workspace ESM: import local from \"./helper.mjs\"; import config from \"./config.json\"; import workspacePackage from \"lodash\";",
        "- Generated inline CommonJS only: const registryPackage = require(\"npm:lodash@4.17.21\"); const scopedSubpath = require(\"npm:@scope/package@1.2.3/subpath\"); const compactRemote = require(\"https://cdnjs.cloudflare.com/ajax/libs/dayjs/1.11.13/dayjs.min.js\");",
        "- Generated inline ESM only: import registryPackage from \"npm:lodash@4.17.21\"; import scopedSubpath from \"npm:@scope/package@1.2.3/subpath\"; import { nanoid } from \"https://cdn.jsdelivr.net/npm/nanoid@5.1.5/index.browser.js\";",
        "Prefer a version-pinned cdnjs or equivalent single-file build for small, self-contained tasks. Do not download a full npm package for a trivial helper when a compact browser build provides the required API. When an npm package is necessary, prefer the lightest package that provides the required functionality and avoid large packages or heavy dependency trees. In workspace scripts, use standard bare package names and pass missing Registry packages through dependencies. Reserve non-standard npm: and direct HTTP/HTTPS module specifiers for generated inline code. Every prepared dependency must be a string literal. CommonJS must use require(), not import(); ESM may use static import or a string-literal dynamic import().",
      ].join("\n"),
      ...
      [
        ...
        "- Core and data: node:assert, node:assert/strict, node:buffer, node:events, node:path, node:path/posix, node:process, node:punycode, node:querystring, node:string_decoder, node:url, node:util, and node:util/types.",
        ...
        "node:process is limited to argv, env, cwd(), exit(), exitCode, and nextTick(). node:http and node:https provide only minimal get/request plus data, end, and error events; prefer fetch, and do not use setEncoding, resume, Agent, sockets, or connection-pool APIs. node:zlib supports buffered deflate/inflate, raw deflate/inflate, gzip/gunzip, and unzip APIs but not Brotli or full native streaming semantics.",
