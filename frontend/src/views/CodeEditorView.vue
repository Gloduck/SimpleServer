  ...
  return [
    ...
    { type: "function", name: "run_javascript", description: aiToolDescription(
      ...
      [
        ...
        "Prefer a version-pinned cdnjs or equivalent single-file build for small, self-contained tasks. Do not download a full npm package for a trivial helper when a compact browser build provides the required API. Use bare package names only for workspace node_modules. Use npm: to download Registry packages. Every prepared dependency must be a string literal. CommonJS must use require(), not import(); ESM may use static import or a string-literal dynamic import().",
      ].join("\n"),
